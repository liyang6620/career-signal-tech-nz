  ...
  return (
    <div className="product-shell">
      ...
      <main>
        ...
        {view === "graph" && <div className="tool-page"><header><p>Evidence graph</p><h1>What your profile can prove</h1><span>Only confirmed evidence contributes to this view.</span></header><section className="tool-panel">{roleFit ? <><div className="fit-summary"><div><span>Documented fit</span><strong>{roleFit.score}</strong><small>/ 100</small></div><div><span>Coverage</span><strong>{roleFit.coverage}%</strong></div><div><span>Evidence depth</span><strong>{roleFit.evidence_depth}%</strong></div></div><div className="evidence-network"><div className="network-role"><Target size={18} /><b>{role ? roles[role] : "Target role"}</b><small>target role</small></div>{roleFit.contributions.map((item) => <div className={`network-skill ${item.evidence_level ? "supported" : "missing"}`} key={item.skill_slug}><span>{item.skill_name}</span><small>{item.evidence_level ? `Level ${item.evidence_level}/5` : "No evidence"}</small>{item.required && <em>required</em>}</div>)}</div></> : <div className="market-empty"><FileText size={24} /><h2>Create your evidence profile first</h2><p>Complete Workspace setup and confirm CV or GitHub evidence to populate this graph.</p><button className="primary-button" onClick={() => setView("workspace")}>Open workspace</button></div>}</section></div>}
